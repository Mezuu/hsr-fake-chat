...
export function MessageBox({ index, name, imageSrc, isSender, message, updateMessage }:
    ...
    function Message() {
        return <Flex className={`px-4 py-0 relative flex-col`}>
            ...
            <Box className="w-full">
                <Center
                    className={[`mt-1 lg:mt-2 py-1 px-3 rounded-xl w-max text-base lg:text-lg max-w-[16em] md:max-w-lg lg:max-w-xl
                ...
                text-hsr-gray-dark`, isSender ? 'rounded-tr-none float-right' : 'rounded-tl-none'].join(" ")}>
                    <Editable className="break-words" defaultValue={message} onChange={handleMessageChange}>
                        <EditablePreview />
                        <EditableInput />
                    </Editable>
                </Center>
    ...
    return <Flex
        className="py-1 lg:py-2 px-4" direction={isSender ? 'row-reverse' : 'row'}
